...
import type { AgentDto, CreateOrUpdateAgentRequest } from "@/types/agent.type";
...
export const AgentForm = ({ agent }: AgentFormProps) => {
	...
	const targetUrlPath = agent
		? `/agentServer/agents/${agent.id}`
		: "/agentServer/agents";

	const agentMutation = useMutation({
		mutationFn: (data: CreateOrUpdateAgentRequest) =>
			fetch(targetUrlPath, {
	...
	const handleSubmit = (data: AgentFormData) => {
		const submitData: CreateOrUpdateAgentRequest = {
			...data,
			webhookUrl: data.webhookUrl || undefined,
		};
		agentMutation.mutate(submitData);
	};
		...
		<Card className="w-full max-w-2xl mx-auto">
			<CardHeader>
				<div className="flex items-center gap-2">
					...
					<CardTitle>
						{agent ? "Update The Agent" : "Create New Agent"}
					</CardTitle>
				</div>
				<CardDescription>
